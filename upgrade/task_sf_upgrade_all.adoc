...
summary: 您可以升级 NetApp SolidFire 全闪存存储系统及其所有组件。 
keywords: netapp, solidfire, cluster, upgrade, element 
---
= 升级适用于 Element 12.3.x 的 NetApp SolidFire 全闪存存储系统


[role="lead"]
您可以通过按顺序升级所有 SolidFire 存储组件，在部署后使 NetApp Element 存储系统保持最新。

这些组件包括管理服务， HealthTools ， NetApp Hybrid Cloud Control （ HCC ）， Element 软件，管理节点以及适用于 vCenter Server 的 Element 插件（取决于您的安装）。
