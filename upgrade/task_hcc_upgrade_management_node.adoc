...
[role="lead"]
您可以将管理节点从 11.0 或更高版本升级到管理节点 12.3 版。
...
* 如果要从管理节点 12.2 升级：<<Upgrade a management node to version 12.3 from 12.2>>
* 如果要从管理节点 12.0 升级：<<Upgrade a management node to version 12.3 from 12.0>>
* 如果要从管理节点 11.3 ， 11.5 ， 11.7 或 11.8 升级：<<Upgrade a management node to version 12.3 from 11.3 through 11.8>>
* 如果要从管理节点 11.0 或 11.1 升级：<<Upgrade a management node to version 12.3 from 11.1 or 11.0>>
...
== 将管理节点从 12.2 升级到版本 12.3

您可以将管理节点从 12.2 版原位升级到 12.3 版，而无需配置新的管理节点虚拟机。


NOTE: Element 12.3 管理节点是一种可选升级。现有部署不需要此功能。

.您需要什么？ #8217 ；将需要什么
* 管理节点 VM 的 RAM 为 24 GB 。
* 您要升级的管理节点为 12.0 版，并使用 IPv4 网络。管理节点 12.3 版不支持 IPv6 。
+
...
* 如果要将管理节点更新到版本 12.3 ，则需要使用管理服务 2.14.60 或更高版本继续操作。
...
== 将管理节点从 12.0 升级到版本 12.3

您可以将管理节点从 12.0 版原位升级到 12.3 版，而无需配置新的管理节点虚拟机。
...
== 将管理节点从 11.3 升级到 11.8

您可以将管理节点从 11.3 ， 11.5 ， 11.7 或 11.8 原位升级到 12.3 版，而无需配置新的管理节点虚拟机。


NOTE: Element 12.3 管理节点是一种可选升级。现有部署不需要此功能。

.您需要什么？ #8217 ；将需要什么
* 您要升级的管理节点为 11.3 ， 11.5 ， 11.7 或 11.8 版，并使用 IPv4 网络。管理节点 12.3 版不支持 IPv6 。
+
...
== 将管理节点从 11.1 或 11.0 升级到版本 12.3

您可以将管理节点从 11.0 或 11.1 原位升级到版本 12.3 ，而无需配置新的管理节点虚拟机。
...
* 您要升级的管理节点为 11.0 或 11.1 版，并使用 IPv4 网络。管理节点 12.3 版不支持 IPv6 。
+
...
. 在 12.3 管理节点上，运行 `upgrade-mnode` 脚本以保留先前的配置设置。
+
...
. （对于使用适用于 vCenter Server 的 NetApp Element 插件安装的所有 NetApp SolidFire 全闪存存储）按照中的步骤更新 12.3 管理节点上的 vCenter 插件 link:task_vcp_upgrade_plugin.html["升级适用于 vCenter Server 的 Element 插件"] 主题。
