...
在遵循此操作步骤之前，您应了解 link:../concepts/concept_solidfire_concepts_volumes.html#persistent-volumes["永久性卷"] 以及是否要使用它们。永久性卷是可选的，但建议在虚拟机丢失时用于管理节点配置数据恢复。
...
----
.. 通过虚拟机管理程序禁用与主机的时间同步（以下是 VMware 示例）：
+
