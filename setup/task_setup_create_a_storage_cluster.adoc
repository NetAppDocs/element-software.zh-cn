...
IMPORTANT: 如果要启用集群范围的 link:../concepts/concept_solidfire_concepts_security.html#encryption-at-rest-software["空闲软件加密"]，您必须使用创建集群  而是 API 方法，并将 enableSoftwareEncryptionAtRest 参数更改为 `true` 。启用空闲软件加密后，无法在集群上禁用它。默认情况下， SolidFire ESDS 集群会启用空闲软件加密。基于硬件的空闲加密可以是  创建集群后。
